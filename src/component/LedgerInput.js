import React, {useState} from 'react';
import styles from './css/LedgerInputWrapper.css';
import {v4 as uuidv4} from 'uuid';

export default function LedgerInput() {
    const [incExpValue, setIncExpValue] = useState('expense');
    const [incExpType, setIncExpType] = useState('foodBev');
    const [account, setAccount] = useState('payme');
    const [amount, setAmount] = useState(0);
    const [remarks, setRemarks] = useState('');
    
    var curr = new Date();
    const [transDate, setTransDate] = useState(curr);
    const [entries, setEntries] = useState([]);

    
    /*function handleChangeIncExpValue(event) {
        console.log(event);
        const nextIncExpValue = event.target.value;
        setIncExpValue(nextIncExpValue);
        console.log('incExpValue = '+incExpValue);
    }

    function handleChangeIncExpType(event) {
        const nextIncExpType = event.target.value;
        setIncExpType(nextIncExpType);
        console.log('incExpType = '+incExpType);
    }

    function handleChangeAccount(event) {
        const nextAccount = event.target.value;
        setAccount(nextAccount);
        console.log('Account = '+ account);
    }*/

    function handleChange(event) {
        const nextValue = event.target.value;
        if(event.target.name === 'transDate') {
            setTransDate(nextValue);
        } else if(event.target.name === 'incomeExpense') {
            setIncExpValue(nextValue);
        } else if(event.target.name === 'incExpType') {
            setIncExpType(nextValue);
        } else if(event.target.name === 'account') {
            setAccount(nextValue);
        } else if(event.target.name === 'amount') {
            if(nextValue.match(/\./g)){
                const [, decimal] = nextValue.split('.');
                if(decimal?.length > 2) {
                    return;
                }
            }
            setAmount(nextValue);
        } else if(event.target.name === 'remarks') {
            const strArr = nextValue.split('');
            if(strArr.length >= 200) {
                console.log(strArr.length);
                const str = strArr.slice(0,200).join('');
                setRemarks(str);
                return;
            }
            setRemarks(nextValue);
        }
        console.log('name = '+event.target.name);
        console.log('value = '+event.target.value);
    }

    function handleSubmit(event) {
        event.preventDefault();
        console.log(event);
        /*if(!transDate || !incExpValue || !incExpType|| !account || !amount) {    
            console.log('Please input all required fields first');
            return;
        }*/
        
        var createdDateTime = new Date();
        const nextEntry = {id: uuidv4(), createDateTime: createdDateTime, transDate: transDate, incExpValue: incExpValue, incExpType: incExpType, account: account, amount: amount, remarks: remarks};
        const nextEntries = [...entries, nextEntry];
        setEntries(nextEntries);
        console.log(nextEntries);




        
    }

    return (
            <form onSubmit={handleSubmit}>
                <div className={styles.field} id="transDate">
                    <div className={[styles.fieldName, styles.required].join(' ')}>Transaction Date</div>
                    <div className="inputValue">
                        <input type="date" name="transDate" value={transDate} onChange={handleChange} ></input>
                    </div>
                </div>
                <div className={styles.field} id="incomeExpense">
                    <div className={[styles.fieldName, styles.required].join(' ')}>Income / Expense</div>
                    <div className="inputValue">
                        <select name="incomeExpense" value={incExpValue} onChange={handleChange}>
                            <option value="expense">Expense</option>
                            <option value="income">Income</option>
                        </select>
                    </div>
                </div>
                <div className={styles.field} id="incExpType">
                    <div className={[styles.fieldName, styles.required].join(' ')}>Income / Expense Type</div>
                    <div className="inputValue">
                        <select name="incExpType" value={incExpType} onChange={handleChange}>
                            <option value="adjustment">Adjustment</option>
                            <option value="bankTransfer">Bank Transfer</option>
                            <option value="cashWithdraw">Cash Withdrawal</option>
                            <option value="donation">Donation</option>
                            <option value="foodBev">Food & Beverage</option>
                            <option value="groceries"s>Groceries</option>
                            <option value="learning">Learning</option>
                            <option value="openBal">Opening Balance</option>
                            <option value="octopusTopUp">Octopus Top Up</option>
                            <option value="others">Others</option>
                            <option value="paymeTopUp">Payme Top Up</option>
                            <option value="subscription">Subscription</option>
                            <option value="tngTopUp">Tap & Go Top Up</option>
                            <option value="transportation">Transportation</option>
                        </select>
                    </div>
                </div>
                <div className={styles.field} id="account">
                    <div className={[styles.fieldName, styles.required].join(' ')}>Account</div>
                    <div className="inputValue">
                        <select name="account" value={account} onChange={handleChange}>
                            <option value="cash">Cash</option>
                            <option value="ccbCreditCard">CCB Credit Card</option>
                            <option value="hsbcAccount">HSBC Account</option>
                            <option value="hsbcCreditCard">HSBC Credit Card</option>
                            <option value="octopus">Octopus</option>
                            <option value="payme">Payme</option>
                            <option value="tng">Tap & Go</option>
                        </select>
                    </div>
                </div>
                <div className={styles.field} id="amount">
                    <div className={[styles.fieldName, styles.required].join(' ')}>Amount (HKD)</div>
                    <div className="inputValue">
                        <input type="number" step=".01" name="amount" value={amount} onChange={handleChange} ></input>
                    </div>
                </div>
                <div className={styles.field} id="remarks">
                    <div className={styles.fieldName}>Remarks</div>
                    <div className="inputValue">
                        <input type="text" name="remarks" value={remarks} onChange={handleChange} ></input>
                    </div>
                </div>
                <button name="Submit" type="submit">Submit</button>
            </form>
    )
}